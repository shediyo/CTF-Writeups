...
def bytes_to_uint_16(rbytes):
    return sum([byte_p * (256 ** (1 - byte_i)) for byte_i, byte_p in enumerate(rbytes)])

def uint_16_to_array(put_array, offset, input_int):
    for i in range(2):
        put_array[offset + i] = (input_int / (256 ** (1 - i))) % 256 

def bytes_to_uint_32(rbytes):
    return sum([byte_p * (256 ** (3 - byte_i)) for byte_i, byte_p in enumerate(rbytes)])

def uint_32_to_array(put_array, offset, input_int):
    for i in range(4):
        put_array[offset + i] = (input_int / (256 ** (3 - i))) % 256 

class multi_value_holder:
    def __init__(self, sm):
        self.regs = sm.regs
        self.value = sm.state_get_uint_8()    
        self.value_type = sm.state_get_uint_8()        
        self.is_set = sm.state_get_uint_8()
    
    def get_value(self):
        if self.value_type ==  1:
            res = self.regs[self.value] * self.is_set
        if self.value_type ==  2:
            res = bytes_to_uint_16(self.regs[self.value : self.value + 2]) * self.is_set
        if self.value_type ==  4:
            res = bytes_to_uint_32(self.regs[self.value : self.value + 4]) * self.is_set   
        print '\t regs get value in offset: ' + str(self.value) + ' with value: ' + str(res)
        return res 
    
    def set_value(self, value_to_set):
        print '\t regs set value: ' + str(value_to_set) + ' in offset: ' + str(self.value) + ' length: ' + str(self.value_type)
        if self.value_type ==  1:
           self.regs[self.value] = value_to_set
        if self.value_type ==  2:
           uint_16_to_array(self.regs, self.value, value_to_set)
        if self.value_type ==  4:
           uint_32_to_array(self.regs, self.value, value_to_set)
        self.is_set = 1

class unset_uint_32_holder:
    def __init__(self, sm):
        self.unset_value = sm.state_get_uint_32()
    
    def get_value(self):
        print '\t immediate get value: ' + str(self.unset_value)
        return self.unset_value
    
    def set_value(self, value_to_set):
        print '\t immediate set value (pass)'
        pass

class summed_value_uint_8:
    def __init__(self, sm):
        print '\t summed value uint 8!'
        self.value = 0
        values_sum_num = sm.state_get_uint_8() 

        while values_sum_num > 0:
            values_sum_num -= 1
            self.value += sm.get_state_params().get_value()
        
        self.value &= 0xFFFFFFFF
        
        self.memory = sm.memory
        self.regs = sm.regs

    def get_value(self):
        print '\t memory get value for offset: ' + str(self.value) + ' with value: ' + str(self.memory[self.value])
        return self.memory[self.value]
    
    def set_value(self, value_to_set):
        print '\t memory set value for offset: ' + str(self.value) + ' with value: ' + str(value_to_set)
        self.memory[self.value] = value_to_set

class summed_value_uint_32:
    def __init__(self, sm):
        print '\t summed value uint 32!'
        self.value = 0
        values_sum_num = sm.state_get_uint_8() 
    ...
    def get_value(self):
        print '\t memory get value for offset: ' + str(self.value) + ' with value: ' + str(bytes_to_uint_32(self.memory[self.value : self.value + 4]))
        return bytes_to_uint_32(self.memory[self.value : self.value + 4])
    
    def set_value(self, value_to_set):
        print '\t memory set value for offset: ' + str(self.value) + ' with value: ' + str(value_to_set)
        uint_32_to_array(self.memory, self.value, value_to_set)
        if self.value >= 10188 and self.value <= 11208:
            global_mem.append(value_to_set)
        if self.value == 11208:
            print '\t' + str(global_mem)


class state_machine:
    def __init__(self, mem_buffer):
        self.memory = mem_buffer
        self.ip = 0
        self.regs = [0] * 32
        uint_32_to_array(self.regs, 16, len(mem_buffer))
        self.zf = False;
        self.cf = False;        
    
    def state_get_uint_32(self):
        get_val = bytes_to_uint_32(self.memory[self.ip : self.ip + 4])
        self.ip += 4
        return get_val        
    
    def state_get_uint_8(self):
        get_val = self.memory[self.ip];
        self.ip += 1
        return get_val
    
    def get_state_params(self):
        choice = self.state_get_uint_8()
        if choice == 1:
            return multi_value_holder(self)       
        if choice == 2:
            return unset_uint_32_holder(self)
        if choice == 3:
            return summed_value_uint_8(self)
        if choice == 4:
            return summed_value_uint_32(self)
        return None
    
    
    def Push(self, push_val):
        esp = bytes_to_uint_32(self.regs[16 : 20]) - 4
        uint_32_to_array(self.regs, 16, esp)
        uint_32_to_array(self.memory, esp, push_val)
        return esp
    
    def Pop(self):
        esp = bytes_to_uint_32(self.regs[16 : 20])
        popped_val = bytes_to_uint_32(self.memory[esp : esp + 4])
        uint_32_to_array(self.regs, 16, esp + 4)       
        return popped_val

    def next_state(self):
        print '\n\ncurrent ip: ' + str(self.ip)
        saved_ip = self.ip
        opcode = self.state_get_uint_8()
        if opcode in [7, 8, 9, 16]: # jump
            print "jump command:"

            cmd_val = self.get_state_params();
            jump_cond = True

            if opcode == 7:
                print '\tjnz'
                jump_cond = (self.zf == False)
            elif opcode == 9:
                print '\tjnc'
                jump_cond = (self.cf == False)
            elif opcode == 16:
                print '\tjz'
                jump_cond = (self.zf == True)
            else:
                print '\tjmp'

            if jump_cond and saved_ip not in [897]:
                self.ip = cmd_val.get_value()

            if saved_ip in [897]:
                uint_32_to_array(self.regs, 0, 870690368)

            if saved_ip in [1002]:
                pass#uint_32_to_array(self.memory, 11284, 37)

            pass # jump
        elif opcode in [3, 5, 6, 11, 13, 17, 18]: # ops
            print 'operation command:'
            cmd_1 = self.get_state_params()
            cmd_2 = self.get_state_params()
            cmd_val_1 = cmd_1.get_value()
            cmd_val_2 = cmd_2.get_value()
            if (opcode == 3) or (opcode ==6):
                print '\t-'
                res_val = cmd_val_1 - cmd_val_2
            if opcode == 5:
                print '\t+'
                res_val = cmd_val_1 + cmd_val_2
            if opcode == 11:
